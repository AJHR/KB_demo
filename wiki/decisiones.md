...
## D-011 — Modo operativo: keyless (decisión del usuario, 2026-06-13)

- **Decisión del usuario:** operar sin credencial del Coordinador por ahora. No se registra `user_key`.
- **Qué queda operando (real, automático):** el ETL nocturno (`etl_nocturno.yml`, 02:00 Chile) mantiene frescas las fuentes sin credencial, que ya tienen backfill real 2019-2026 en la rama default:
  - `calendario` (2.904 filas), `combustibles` (8.997 filas: brent, carbón API2, diésel, Henry Hub, USD/CLP), `clima_observado` (29.931 filas, 11 puntos ERA5), `clima_pronostico` (29.931 filas, Open-Meteo previous_day2). `cen_po` (scraping del listado de Programas de Operación) y `cen_web` (monitor keyless) también corren keyless.
- **Qué queda dormido:** las 5 fuentes de API CEN (`cen_cmg_real`, `cen_cmg_programado`, `cen_demanda`, `cen_generacion`, `cen_embalses`) se auto-omiten cada noche (`omitida_sin_credencial`, sin disparar el issue automático). Sin ellas no hay variable objetivo real, así que **la tabla maestra real no se construye** y los baselines/autoresearch siguen sobre el **dataset sintético etiquetado** (D-009) como sustrato de demostración de la maquinaria.
- **Consecuencia honesta:** en modo keyless el sistema NO produce predicciones reales del costo de operación (el target depende del CEN). Lo que opera es el pipeline de features exógenas reales + la maquinaria validada sobre sintético. Esto está declarado en `index.md` (Estado operativo) y en los reportes (`origen_datos`).
- **Cómo activar el CEN después (sin tocar código):** crear el secret `COORDINADOR_USER_KEY` en GitHub y disparar `backfill.yml`; las fuentes CEN se activan solas, se construye la tabla maestra real y los baselines/autoresearch pasan a datos reales. Reversible en cualquier momento.

## D-012 — Fuente del CMg real y cobertura medida de la API SIP v4 (2026-06-14)

- **Contexto:** con el `COORDINADOR_USER_KEY` cargado, el backfill real reveló que las fuentes CEN pesadas chocaban con el límite duro de 6 h por job de GitHub Actions. Un diagnóstico instrumentado (paginación: páginas, latencia, tope) sobre 1 mes midió la causa **y** destapó un problema de cobertura del CMg real. Evidencia dura (medida en vivo, no inferida), runs #27484278704 / #27484796013 / #27486078701 / #27486313895:
  - `cen_cmg_programado`: 271.138 filas crudas/mes, 273 páginas a `limit=1000`. Fix: `limit=4000` → ~68 páginas (la API respeta el `limit`); 12 → 3,7 min/mes.
  - `cen_embalses`: pedía 1 request/día **sin `limit`** → la API paginaba de a ~19 filas (15 págs/día). Fix: `limit=1000` → 1 pág/día; 13 → 2,3 min/mes.
  - `cen_cmg_real` (variable objetivo): `costo-marginal-real/v4/findByDate` **solo entrega el régimen "nuevo" (≥2024-07-15)** — ago 2024 trajo datos, ene 2024 → 0 filas. `costo-marginal-online/v4` resultó **solo-reciente** (ventana móvil): may 2026 → datos, feb 2024 → 0 filas. El CMg real **anterior** al 2024-07-15 NO está en la API v4 (vive en open data CNE `datos.energiaabierta.cl` o CSV legacy del CEN).
- **Corrección honesta:** una sesión anterior asumió que `costo-marginal-online` cubría ≥2018; es falso (medido). Ese supuesto motivó una primera elección de fuente que se revirtió.
