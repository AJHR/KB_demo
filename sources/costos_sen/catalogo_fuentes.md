...
|---|--------|----------------|--------|------|---------|--------------|----------|---------------|-------------------------------|--------------|
| 1 | **CEN — Programa de Operación diario (costo programado, objetivo-insumo)** | coordinador.cl/operacion/documentos/programas-de-operacion/ · programa.coordinador.cl | Descarga ZIP (tramos ≤30 días) | No | ZIP (PLEXOS CSV/XLSX, COSTOSVARIABLES) | Diaria (horaria por central) | ≥2017-2019 (confirmado por revisión adversarial; mejor que lo declarado inicialmente) | Diaria + PID intradiaria | **SIN CONFIRMAR** — riesgo nº1 del proyecto; la NT 2025 (Res. Ex. 151) delega los horarios al Coordinador → solo verificable empíricamente. Usar siempre la *primera* versión del PO, nunca re-publicaciones PID | pendiente (runner) |
...
| 4 | **CEN — CMg programado (PO/PID)** | `cmg-programado-pid/v4/findByDate` · CSV web | API JSON / CSV | `user_key` / No | JSON/CSV | Horaria, D+1 | **PID solo desde 2022-12 (indicativo) / 2024-07 (regular)** — corregido por revisión adversarial; el CMg programado pre-PID vive en la descarga web del PO | Diaria | Hereda la hora del PO (ver #1) | pendiente (runner) |
...
| 13 | **Combustibles — futuros (sin key)** | stooq.com CSV (`cb.f`, `ng.f`, `ho.f`) · yfinance (`BZ=F`, `NG=F`, `HO=F`, `MTF=F`, `TTF=F`) | CSV directo / lib | No | CSV/JSON | Diaria (cierre) | >10 años | Diaria | Settlement NYMEX 14:28-14:30 ET (corregido: 17:00 ET es cierre Globex) = 15:30-17:30 Chile → ✅ usable mismo día con holgura; carbón `MTF=F` con huecos por iliquidez | pendiente (runner) |
...
Decisión y justificación completa en `wiki/decisiones.md` D-004. Estrategia de implementación en dos etapas: el backfill captura #1 y #6 + costos variables; mientras el parser del ZIP del PO no esté validado contra muestras reales, la tabla maestra usa como objetivo provisional la **valorización de retiros a CMg** Σₕ(demanda_h × CMg_h barra de referencia), reemplazable por columna sin tocar el resto del pipeline.

> ⚠️ **Advertencia de la revisión adversarial (C1)**: la valorización Σ demanda×CMg **no es** el costo de operación — incluye rentas inframarginales (sobreestima en horas térmicas), colapsa hacia 0 en horas de vertimiento solar (subestima al ignorar mínimos técnicos, costos de encendido y SSCC, ~+10 USD/MWh según Systep), y su sesgo es bidireccional y no estacionario con la penetración solar. Es un placeholder técnico para validar la maquinaria, NO un objetivo defendible ante el CEN. El reemplazo por el costo PLEXOS del PO (programado) + costo real reconstruido es **prioridad 1 post-backfill**. Detalle: `anexos/revision_adversarial.md`.
...
2. **Quiebre de régimen el 2024-07-15** — afecta potencialmente a las TRES familias de CMg (real "nuevo", CMg en Línea "nuevo" y entrada del PID regular el mismo día; corrección de la revisión adversarial). Los endpoints v4 podrían servir solo las series nuevas. Requiere armonización en el ETL y flag de régimen en las tres.
